...
boost_cpp_libraries_install()
{
	...
	wget https://dl.bintray.com/boostorg/release/1.64.0/source/boost_1_64_0.tar.gz &> /dev/null || error
...
srilm_install()
{
	...
	wget --no-check-certificate 'https://www.dropbox.com/s/mnfgpaw0oyh81gy/srilm%20%281%29.zip?dl=1' -O srilm.zip &> /dev/null || error
...
advanced_options()
{
  ...
  selection=$(dialog \
    ...
    2 srilm off  \
    3 irstlm on \
    4 "moses and moses2/moses-server and srilm with boost and giza++ " off \
