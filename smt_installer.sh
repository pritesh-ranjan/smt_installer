...
indic_nlp_library_install()
{
	...
	spinner &
	git clone https://github.com/anoopkunchukuttan/indic_nlp_library.git &> /dev/null
...
{		
	...
	wget https://dl.bintray.com/boostorg/release/1.64.0/source/boost_1_64_0.tar.gz &> /dev/null
	echo 'unpacking boost'
	tar zxvf boost_1_64_0.tar.gz &> /dev/null
	cd boost_1_64_0/
...
giza_pp_install()
{
	...
	git clone https://github.com/moses-smt/giza-pp.git &> /dev/null
	cd giza-pp
	echo 'Compiling giza++'
	make &> /dev/null || error
	cd ../
...
irstlm_install()
{
	...
	wget -O irstlm.zip https://sourceforge.net/projects/irstlm/files/irstlm/irstlm-5.80/irstlm-5.80.08.zip/download &> /dev/null
...
srilm_install()
{
	...
	wget --no-check-certificate 'https://www.dropbox.com/s/mnfgpaw0oyh81gy/srilm%20%281%29.zip?dl=1' -O srilm.zip &> /dev/null
...
xmlrpc_install()
{
	...
	wget -O xmlrpc-c-1.39.12.tgz https://sourceforge.net/projects/xmlrpc-c/files/Xmlrpc-c%20Super%20Stable/1.39.12/xmlrpc-c-1.39.12.tgz/download &> /dev/null
...
moses_install()
{
	...
	./bjam --with-boost=$wdirect/boost_1_64_0 --with-giza-pp=$wdirect/bin -j4 &> /dev/null
	./bjam --with-boost=$wdirect/boost_1_64_0 --with-irstlm=$wdirect/irstlm --with-giza-pp=$wdirect/bin -j4 &> /dev/null 
...
moses2_install()
{
	...
	./bjam --with-giza-pp=$HOME/bin -j4 --with-xmlrpc-c=$wdirect/xmlrpc -j4  &> /dev/null
	./bjam --with-boost=$wdirect/boost_1_64_0 --with-giza-pp=$wdirect/bin --with-xmlrpc-c=$wdirect/xmlrpc --with-srilm=$wdirect/srilm -j4
	./bjam --with-boost=$wdirect/boost_1_64_0 --with-irstlm=$wdirect/irstlm --with-giza-pp=$wdirect/bin --with-xmlrpc-c=$wdirect/xmlrpc --with-srilm=$wdirect/srilm -j4

	sudo cp bin/build_binary /usr/local/bin/build_binary
	sudo cp bin/moses /usr/local/bin/moses2
	sudo cp scripts/recaser/train-truecaser.perl /usr/local/bin/train-truecaser.perl
	sudo cp scripts/tokenizer/tokenizer.perl /usr/local/bin/tokenizer.perl 
	sudo cp scripts/recaser/truecase.perl /usr/local/bin/truecase.perl
	sudo cp scripts/training/clean-corpus-n.perl /usr/local/bin/clean-corpus-n.perl
...
test_moses()
{
	...
	wget http://www.statmt.org/moses/download/sample-models.tgz  &> /dev/null || chk_internet_connection 2> /dev/null
...
sudo apt-get install -y dialog &> /dev/null || chk_internet_connection 2> /dev/null
...
advanced_options()
{
  ...
  selection=$(dialog \
    ...
    2 srilm off  \
    3 irstlm on \
    4 "moses, moses2/moses-server, srilm with boost and giza++ " off \
    5 "mosesdecoder with boost, giza++ and kenlm" on \
    2>&1 1>&3)
  exit_status=$?
  exec 3>&-
  case $exit_status in
